...
from qstrader.event import (SignalEvent, BarEvent)
            ...
            if all(self.latest_prices > -1.0):
                ...
                if self.days > 1:
                    ...
                    if self.invested is None:
                        cur_hedge_qty = int(floor(self.qty*self.theta[0]))
                        if et < -sqrt_Qt:
                            # Long Entry
                            print("LONG: %s" % event.time)
                            self.events_queue.put(SignalEvent(self.tickers[1], "BOT", self.qty))
                            self.events_queue.put(SignalEvent(self.tickers[0], "SLD", cur_hedge_qty))
                            self.invested = "long"
                        elif et > sqrt_Qt:
                            # Short Entry
                            print("SHORT: %s" % event.time)
                            self.events_queue.put(SignalEvent(self.tickers[1], "SLD", self.qty))
                            self.events_queue.put(SignalEvent(self.tickers[0], "BOT", cur_hedge_qty))
                            self.invested = "short"
                    # If we are in the market...
                    if self.invested is not None:
                        cur_hedge_qty = self.qty
